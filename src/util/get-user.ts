...
export const getUserGitHub = async (): Promise<GithubUser> => {
    ...
    const [repositoriosPage1Response, repositoriosPage2Response, repositoriosPage3Response] = await Promise.all([
    ...
    if (!repositoriosPage1Response.ok || !repositoriosPage2Response.ok || !repositoriosPage3Response.ok) {
        throw new Error('Failed to fetch repositories data');
    }

    // Obter os dados em JSON das três requisições
    const [repositoriosPage1, repositoriosPage2, repositoriosPage3] = await Promise.all([
    ...
    const projectsInitials = repositorios.filter((repo) => initialRepositories.some((padrao: any) => repo.name.includes(padrao)));
    const projects = repositorios.filter((repo) => repositories.some((padrao: any) => repo.name.includes(padrao)));
